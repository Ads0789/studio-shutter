...
>(({ data, totals }, ref) => {
  ...
  // A4 size in mm: 210 x 297. We'll use this as a reference for page breaks.
  // We'll give some margin for the footer.
  return (
    <div ref={ref} className="bg-white text-black text-sm p-12 font-serif" style={{width: '210mm'}}>
      {/* Watermark */}
      ...
      {/* Main Content: Set to min-height of A4 to allow content to flow naturally */}
      <div className="relative z-10" style={{minHeight: '267mm'}}>
        <div className="flex flex-col min-h-full">
          {/* This div wraps content that should ideally stay on one page, but can be split if necessary */}
          <div className="flex-grow">
            ...
            {/* The Event Summary section is often the cause of page break issues */}
            {events.length > 0 && (
              <section className="mb-8" style={{ pageBreakBefore: 'auto', pageBreakInside: 'avoid' }}>
                <h3 className="font-bold uppercase text-neutral-600 border-b pb-1 mb-2">Event Summary</h3>
                {events.map(event => (
                  // Each event is a block that we want to keep together
          ...
          {/* Footer that sticks to the bottom of the content flow */}
          <footer className="text-xs text-neutral-600 space-y-4 pt-8 border-t border-neutral-200 mt-auto" style={{ pageBreakInside: 'avoid' }}>
